...
exports.getAppointments=async (req,res,next)=>{
    ...
    try {
        ...
        console.log(error);
        return res.status(500).json({success:false, message:"Cannit find Appointment"});
    }
}
...
exports.addAppointment= async (req,res,next)=>{
    try{
        ...
        if(existedAppointments.length >= 3 && req.user.role !=='admin'){
            return res.status(400).json({success:false,message:`The user witj ID ${req.user.id} has already made 3 appointments`});
